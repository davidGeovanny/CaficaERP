...
namespace CaficaERP
{
    ...
    public class CrearTicket
    {
        ...
        public void Encabezado(string Documento)
        {
            ...
            if (Documento == "RESGUARDO")
                linea.AppendLine("COMPONENTE                | UNID | CANT ");
        }
        ...
        public void AgregaComponente(string componente,string unidad, double cant)
        {
            //Valida que cant y unidad esten dentro del rango.
            if (cant.ToString().Length <= 6 && unidad.ToString().Length <= 6)
            {
                ...
                if (componente.Length > 25)
                {
                   
                    //Colocar la unidad a la derecha.
                    nroEspacios = (6 - unidad.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                    ...
                    elemento += espacios + unidad.ToString();//Agregamos el precio a la variable elemento
                    ...
                    for (int longitudTexto = componente.Length; longitudTexto > 25; longitudTexto -= 25)
                    {
                        if (bandera == false)//si es false o la primera linea en recorrerer, continuar...
                        {
                            ...
                            linea.AppendLine(componente.Substring(caracterActual, 25) + elemento);
                            bandera = true;//cambiamos su valor a verdadero
                        }
                        else
                            linea.AppendLine(componente.Substring(caracterActual, 20));//Solo agrega el nombre del articulo

                        caracterActual += 25;//incrementa en 20 el valor de la variable caracterActual
                    }
                    //Agrega el resto del fragmento del  nombre del articulo
                    linea.AppendLine(componente.Substring(caracterActual, componente.Length - caracterActual));

                }
                else //Si no es mayor solo agregarlo, sin dar saltos de lineas
                {
                    for (int i = 0; i < (25 - componente.Length); i++)
                    {
                        espacios += " "; //Agrega espacios para completar los 20 caracteres
                    }
                    elemento = componente + espacios;
                    ...
                    elemento += espacios + unidad.ToString();
